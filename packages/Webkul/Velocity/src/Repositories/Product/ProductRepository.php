...
class ProductRepository extends Repository
{
    ...
    public function searchProductsFromCategory($params)
    {
        ...
        $locale = request()->get('locale') ?: app()->getLocale();
        $results = DB::table('product_flat')
            ->select(DB::raw("*"))
            ->where('status', 1)
            ->where('visible_individually', 1)
            ->where('channel', $channel)
            ->where('locale', $locale)
           ->where('name', 'like', '%' . urldecode($term) . '%')
           ->orWhere('sku', 'LIKE', '%'.urldecode($term).'%')
           ->orWhere('meta_keywords', 'LIKE', '%'.urldecode($term).'%')
    ...
    public function searchProductsFromCategory_old_bagisto($params)
    {
        $term = $params['term'] ?? '';
        $categoryId = $params['category'] ?? '';
        ...
        ///    name,sku,price,url_key,thumbnail,product_id,
                           ->addSelect('product_flat.*')
