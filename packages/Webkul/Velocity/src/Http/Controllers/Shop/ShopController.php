...
class ShopController extends Controller
{
    ...
    {
        $results = $this->velocityProductRepository->searchProductsFromCategory(request()->all(),'ajax');
        foreach ( $results as $result ){
            $result->url_key = route('shop.productOrCategory.index', $result->url_key);
         ///   $galleryImages = $this->productImageHelper->getProductBaseImage($result);
         ///   $result->image = $galleryImages['small_image_url'];
         ///   $result->priceHtml = $result->getTypeInstance()->getPriceHtml();
        }
        return response()->json($results);
    }
    public function searchAjax_old_bagisto()
    {
        $results = $this->velocityProductRepository->searchProductsFromCategory(request()->all(),'ajax');
        foreach ( $results as $result ){
            $result->url_key = route('shop.productOrCategory.index', $result->url_key);
    ...
    public function categoryDetails()
    {
        ...
        switch ($slug) {
            ...
            case 'featured-products':
                ...
                break;
            default:
                $categoryDetails = $this->categoryRepository->findBySlugOrFail($slug);
